package com.pragmasoft.bigdata.scalding.elasticsearch
...
import com.pragmasoft.bigdata.scalding.tap.elasticsearch.testjob.WriteToES
...
class EsSourceSpec extends FlatSpec with Matchers {
  ...
  class EmeddedEsTest(input: List[String], test: EmeddedEsTest => Unit) {
    ...
    def runWriteJob(contentFile: File): Unit =
      Tool.main(Array(classOf[WriteToES].getName, "--local", "--input", contentFile.getAbsolutePath))
